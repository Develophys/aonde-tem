...
(denunciar)** a product or sighting as illegal/inappropriate, and **comment** to give feedback. An admin
...
## User stories
- As a contributor, I want to flag a product or sighting (illegal/inappropriate/spam/wrong info) so bad content gets removed.
- As a contributor, I want to comment on a product or sighting (e.g., "já acabou") so I can give feedback.
- As an admin, I want a list of open flags so I can review and remove offending content quickly.

## Requirements

### P0
- **Flag (denúncia)** a product or sighting with a `reason` (+ optional comment); **login required**.
  - *Given* I'm logged in, *when* I flag a target with a reason, *then* a `Flag` is recorded as `open`.
- **Admin removal:** an admin can hide/remove a flagged product/sighting and resolve the flag.
  - *Given* a product gets a valid `illegal` flag, *when* an admin actions it, *then* the product and its sightings are hidden from all read endpoints.
- **Minimal admin surface:** a list of open flags + a hide/resolve action (admin-only endpoint or minimal UI is fine for v1).

### P1
- **Comments** on products and sightings (display + post).
...
## Dependencies
- `Flag` + `Comment` entities & migration (E10). Auth (login gate). Product/Sighting reads must honor hidden status.
